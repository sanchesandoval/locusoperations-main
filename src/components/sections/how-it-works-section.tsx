...
type HowItWorksItem = (typeof siteConfig)["howItWorksSection"]["items"][number];
...
export function HowItWorksSection() {
  ...
  const mainItems = howItWorksSection.items.slice(0, 3);
  const finalItem = howItWorksSection.items[3];

  return (
    <section
      id="how-it-works"
      className="flex flex-col items-center justify-center w-full relative px-5 md:px-10 scroll-mt-36"
    >
      <div className="border-x mx-5 md:mx-10 relative">
        ...
        {finalItem && (
          <motion.div
            ...
            className="border-t border-border relative before:absolute before:-left-0.5 before:top-0 before:z-10 before:h-full before:w-px before:bg-border before:content-['']"
          >
            <StepCard item={finalItem} />
          </motion.div>
        )}
      </div>
    </section>
